...
typedef struct XMLDLLENTRY XMLNode
{
  ...
  public:
    ...
    * @{ */
    ...
    int getChildNodeIndex(XMLNode x) const;                        ///< added by mdavid, return the index of the child node given by x.
    int nChildNode(XMLCSTR name) const;                            ///< return the number of child node with specific name
    int nChildNode() const;                                        ///< nbr of child node

	void selectNodes(const char* xpathString,                      ///< added by mdavid, selects sub nodes based on XPath string/expression criteria.
